...
5.运行main.py文件，根据提示点击游戏窗口区域，注意region=(aaa,bbb,ccc,ddd)，请注意bbb和ccc的值请尽量不要过千，程序使用连线的斜率来检测柱子是否移动，如果过千可能导致无法正常释放柱子，请缩小窗口并再次运行main

```
img = pyautogui.screenshot(region=(aaa, bbb, ccc, ddd))
```

6.在终端中输入以下代码运行主程序，之后再点击模拟器窗口使其成为当前焦点，即可正常使用
